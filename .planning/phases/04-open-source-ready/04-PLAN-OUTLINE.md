...
|---|---|---|---|---|
| 04-01 | Contributor docs — `docs/adding-a-retailer.md` walking Nintendo end to end, why a control product is mandatory, the UNKNOWN contract; `CONTRIBUTING.md`; README prose entry point. Also adds `CONTRIBUTING.md` and `hooks` to the mutation sandbox | 1 | — | REQ-09 |
| 04-02 | The missing `LICENSE`, and packaging metadata that matches it — MIT text, `license-files` wiring, URLs, classifiers, and a test that fails if the declared licence and the shipped file ever disagree. Also gives the mutation sandbox `LICENSE`, `MANIFEST.in` and a git index | 2 | 04-01 | REQ-11 |
| 04-03 | A linter, from zero — `ruff` in the `dev` extra, a curated rule set recorded as a decision, the findings actually resolved, a `lint` stage inside `make verify`, its README stage row, and the stage watched going red | 3 | 04-01, 04-02 | REQ-10 |
| 04-04 | GitHub Actions CI — one job on every PR that runs `make verify-offline` and nothing else, plus an offline test asserting the workflow's contract (offline target, no `.[browser]`, no exit-code flattening, least-privilege permissions) | 4 | 04-03 | REQ-10 |
| 04-05 | Release engineering, all of it local — version 1.0.0, `CHANGELOG.md`, a tag-triggered publish workflow using PyPI Trusted Publishing, and artifacts *proven* by `python -m build` + `twine check` + a wheel installed into a clean venv where `boty --help` runs | 5 | 04-02, 04-03, 04-04 | REQ-11 |
| 04-06 | Maintainer handoff — the one authenticated action this agent cannot take: configure the Trusted Publisher, push `v1.0.0`, confirm `pip install bot-y` from PyPI, then record the criteria verdicts | 6 | 04-05 | REQ-11 |
...
Nintendo is the spine of the doc, because it teaches that **the default answer is "no adapter"**: four edits, all citable in the tree (`docs/retailer-evidence.md:958-979`, `boty/retailers.py:28`, the `MARKETPLACES` absence-is-a-claim comment at `:88-94`, `config/products.yaml:51-70`). Amazon is the second example (the case where `check_html` says UNKNOWN forever and a DOM reader is needed). Files: `docs/adding-a-retailer.md`, `CONTRIBUTING.md`, `README.md`, `tests/test_contributor_docs.py`, and `scripts/mutation_check.py` — the last one because the new gate reads `CONTRIBUTING.md` and cites `hooks/pre-commit`, neither of which the mutation sandbox copies. See **Wave structure** below for why that stopped 04-01 and 04-02 running in parallel.
...
Version stays at `0.1.0` here — the bump belongs with the release in 04-05. The load-bearing comment blocks in `pyproject.toml` (the mypy 2.x strictness rationale at `:22-30` and `:74-87`, the audited-2026-08-02 AGPL `nodriver` note at `:32-49`, the committed-mypy-config and `addopts` notes) are recorded decisions cited from CONTEXT: they survive verbatim, and no TOML formatter runs over this file, ever. Files: `LICENSE`, `MANIFEST.in`, `pyproject.toml`, a new metadata test, and `scripts/mutation_check.py`.

The metadata test's sdist rule reads the tracked-file list from `git ls-files`, which cannot answer inside the mutation sandbox — a plain directory copy with no git index. Returning an empty set there would make `_unpruned_directories` vacuously green, i.e. the sdist gate reporting success precisely where it could not run, which is the same defect as an MIT declaration with no MIT text. So the rule raises a named `NotATrackedTree` instead, and `build_sandbox()` gains a `git init` + `git add -A` so the rule genuinely runs there. The repo's existing skip idiom (`tests/test_identity_check.py` § `needs_repo`) was read and deliberately declined; the reasoning is written into the plan and into the test.
...
**There is no parallel wave.** The original split put 04-01 and 04-02 in wave 1 on the grounds of zero file overlap — 04-01 owning `README.md`, `docs/`, `CONTRIBUTING.md`, 04-02 owning `LICENSE`, `pyproject.toml`. Review found an overlap that had been missed: both plans add a gate that reads a file the **mutation sandbox** does not copy, so both must edit `SANDBOX_CONTENTS` in `scripts/mutation_check.py`. That is genuine contention over a shared harness, and the obvious fix — one plan making a single edit adding all four entries — is not available. `build_sandbox()` raises `HarnessError: cannot build sandbox: … does not exist` for any entry with no file behind it, so whichever plan owned the consolidated edit would name files the other had not created yet and would redden its own `make verify-offline`. **Each entry has to land in the same commit as the file it names.**

So the contention is resolved by wave, which is how this phase already resolves it everywhere else: 04-04 edits both `scripts/mutation_check.py` and `MANIFEST.in` — files 04-02 creates earlier — and that is not contention, because the waves are ordered. 04-01 keeps wave 1 and adds `CONTRIBUTING.md` and `hooks`; 04-02 moves to wave 2, depends on 04-01, and adds `LICENSE` and `MANIFEST.in` plus the sandbox's git index. 04-03 through 04-06 shift one wave later accordingly. The cost is wave 1's parallelism, accepted deliberately: on a solo project a serial wave costs wall-clock time, while two unordered edits to a shared harness cost a red `make verify` that cannot be attributed to either plan.

Waves 2-6 are single-plan by necessity, not by habit. `README.md` and `pyproject.toml` are contested by nearly every plan in this phase, and serialising them here is a **safety property**: `README.md` carries the machine-read support matrix, the tightest coupling in the phase, where two concurrent edits could break `tests/test_support_matrix.py` or silently rebind its seven-column locator to the wrong table. The dependency 04-03 → 04-04 is likewise real rather than bookkeeping — CI cannot lint before a linter exists and its 43 findings are resolved, and landing CI first would simply paint `main` red.
