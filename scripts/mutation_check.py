...
MUTATIONS = (
    ...
    Mutation(
        ...
        # Re-pointed 2026-08-13 with the v0.3 scoping roll, and AGAIN 2026-08-27 with the
        # v0.4 one. The subject is unchanged: the anchor is the declared version literal,
        # so it necessarily moves every time that literal does. The harness refused to run
        # rather than silently dropping to 25 mutations while printing 26 — which is the
        # behaviour that made this a two-minute fix instead of a quiet hole, and it did it
        # a second time on 2026-08-27, exactly as designed. Anchor uniqueness re-measured
        # before this edit, per M19's trap: `grep -o 'version = "0.4.0"' pyproject.toml`
        # -> 1, and the README instruction -> 1.
    ...
    Mutation(
        ident="M26",
        target="README.md",
        search="Publication happens from the `v0.4.0` tag",  # re-pointed with the v0.3 and v0.4 rolls, same reason as M25
